...
def get_sensor_data(item_id,data_item):
    ...
    reading1 = {'id' : item_id,
            'machineId' : 'Machine1',
            'timestamp' : int(data_item[0]),
            'env_temp' : float(data_item[4]),
            'env_hum' : float(data_item[3]),
            'light' : int(data_item[5]),
            'co2' : int(data_item[2]),
            'soil_moisture' : int(data_item[1]),
            'result' : data_item[6],
            'image classification' : data_item[7]
            }
...
def run_sample():
    ...
    rows = []
    with open("sensor_data.csv", 'r', errors='ignore') as file:
        csvreader = file.readlines()
        for row in csvreader[2:]:
